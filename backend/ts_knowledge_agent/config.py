...
class Settings:
    ...
    def with_env_overrides(self) -> "Settings":
        return Settings(
            ...
            os.getenv("TS_KB_GIT_REMOTE", self.git_remote),
            Path(os.getenv("TS_KB_WORKDIR", str(self.workdir))).expanduser(),
        )
...
def clone_knowledge_repo(settings: Settings) -> None:
    ...
    result = subprocess.run(["git", "clone", settings.git_remote, str(settings.knowledge_repo)], capture_output=True, text=True, encoding="utf-8", errors="replace", check=False)
