...
def main(argv: Sequence[str] | None = None) -> int:
    parser = build_parser(); args = parser.parse_args(argv); settings = Settings.from_env()
    ...
    if args.command == "convert":
        source = args.file.expanduser().resolve(); output = args.output or settings.knowledge_repo / "members" / settings.member_id / "converted" / f"{source.stem}.md"
        result = convert_file(source, output); print(f"converted={result.output_path} bytes={result.bytes_written}"); return 0
