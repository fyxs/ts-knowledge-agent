...
from ts_knowledge_agent.services.pipeline import run_once`nfrom ts_knowledge_agent.services.scheduler import run_scheduler
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("run-once")`n    sub.add_parser("schedule")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "schedule":`n        return run_scheduler(settings)`n    if args.command == "status":
        state = StateStore(settings.knowledge_repo / "data" / "state.sqlite3")
        try:
