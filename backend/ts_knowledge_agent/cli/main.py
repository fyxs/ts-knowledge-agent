...
def build_parser() -> argparse.ArgumentParser:
    ...
    init = sub.add_parser("init"); init.add_argument("--workdir", required=True, type=Path); init.add_argument("--member-id", required=True); init.add_argument("--source-root", required=True, type=Path); init.add_argument("--interval-minutes", type=int, default=60); init.add_argument("--git-remote", default=None)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "init":
        try:
            settings = Settings(args.member_id, args.source_root, args.workdir, args.workdir / "knowledge-base" / "ts-knowledge-base", args.interval_minutes, args.git_remote or Settings.from_env().git_remote)
            settings.workdir.mkdir(parents=True, exist_ok=True); clone_knowledge_repo(settings); settings.write_file(settings.workdir / "ts-kb.json")
            print(f"initialized workdir={settings.workdir} knowledge_repo={settings.knowledge_repo} git_remote={settings.git_remote}")
            return 0
    ...
    if args.command == "convert":
        source = args.file.expanduser().resolve(); output = args.output or settings.knowledge_repo / "members" / settings.member_id / "converted" / f"{source.stem}.md"; result = convert_file(source, output); print(f"converted={result.output_path} bytes={result.bytes_written}"); return 0
    ...
    if args.command == "status":
        state = StateStore(settings.knowledge_repo / "data" / "state.sqlite3")
        try: print(f"sources={len(state.list_sources())} conversions={len(state.list_conversions())} member_id={settings.member_id}")
        finally: state.close()
        return 0
